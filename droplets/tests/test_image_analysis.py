...
def test_localization_perturbed_2d(periodic):
    ...
    d1 = PerturbedDroplet2D(pos, radius, interface_width=width,
                            amplitudes=ampls)
...
def test_localization_perturbed_3d(periodic):
    ...
    d1 = PerturbedDroplet3D(pos, radius, interface_width=width,
                            amplitudes=ampls)
    ...
    assert d1.radius == pytest.approx(d2.radius, rel=1e-5)
    assert d1.interface_width == pytest.approx(d2.interface_width, rel=1e-3)
    np.testing.assert_allclose(d1.amplitudes[3:], d2.amplitudes[3:],
                               rtol=0.5, err_msg=msg)
