...
def test_empty_emulsion():
    ...
    for b in [True, False]:
        np.testing.assert_array_equal(e.get_neighbor_distances(b),
                                      np.array([]))
...
def test_emulsion_single():
    ...
    expect = {'count': 1,
              'radius_mean': 3, 'radius_std': 0,
              'volume_mean': 6, 'volume_std': 0} 
    assert e.get_size_statistics() == expect
    for b in [True, False]:
        np.testing.assert_array_equal(e.get_neighbor_distances(b),
                                      np.array([np.nan]))
...
def test_emulsion_two():
    ...
    expect = {'count': 2,
              'radius_mean': 4, 'radius_std': 1,
              'volume_mean': 8, 'volume_std': 2} 
    assert e.get_size_statistics() == expect

    np.testing.assert_array_equal(e.get_neighbor_distances(False),
                                  np.array([10, 10]))
    np.testing.assert_array_equal(e.get_neighbor_distances(True),
                                  np.array([2, 2]))
